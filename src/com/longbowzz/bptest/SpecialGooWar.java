...
public class SpecialGooWar extends JApplet implements Runnable{
	static boolean isApplet=false;
	
	static String INPUT_PATH = "D:\\workspace2\\SpecialGooWar\\input\\";
	static String OUTPUT_PATH = "D:\\workspace2\\SpecialGooWar\\output\\";

	static String WEIGHT_FILE = "war_S20160119_MEM_BLK_WINNER.dat";
	static String PLAYER = "zhangzhen05_MEM_";
	
	static String[] WEIGHTS = {OUTPUT_PATH+"S01_MEM_BLK_humanTrained.dat",
//							 OUTPUT_PATH+"S02_MEM_humanTrained.dat",
//							 OUTPUT_PATH+"S03_MEM_humanTrained.dat",
//							 OUTPUT_PATH+"S04_MEM_humanTrained.dat",
							 OUTPUT_PATH+"S02_MEM_BLK_humanTrained.dat"};
	
	static final boolean DUEL = false; //是否对决, false表示随机个体竞争
...
//  int screenH = 600;
  ...
  int mtt=0;
	...
	int goocnt = TEST_MODE?500:10;
	int WINNER_CNT = 10;
	int SEED_CNT = 0;
		
	int FOOD_CNT = TEST_MODE?100:1;
...
//	MemGoo[] goos = new MemGoo[goocnt];
  ...
  boolean isstart=false;
	...
	public void run(){
		...
		double[][] initGenes = new double[WEIGHTS.length][];
		try {
			for(int i=0;i<WEIGHTS.length;i++){
				initGenes[i] = loadWeights(WEIGHTS[i]);
			}
		...
		for(int i=0; i<goocnt; i++){
			...
			MemGoo goo = null;
			if(initGenes == null || i>=SEED_CNT){
				goo = new MemGoo(IDPool.getID(), getSize().width, getSize().height); //使用随机个体
				
			}else{
				
				int j = i % WEIGHTS.length;
				if(DUEL){
					goo = new MemGoo(-1*(j+1), getSize().width, getSize().height, initGenes[j], true); //使用已保存的个体
					Random rand = new Random(goo.ID);
					rand.nextInt();
					Color color = new Color(rand.nextInt());
					goo.ball.c = color;
				}else
					goo = new MemGoo(-1*IDPool.getID(), getSize().width, getSize().height, initGenes[j], true); //使用已保存的个体					
...
//							System.out.println("OverSpeed("+maxspeed+"): "+goo.v1+", "+goo.v2+" diffV="+diffV+" ,Vxy="+speedCx+","+speedCy);
				...
				if(goo.life>0){
					...
					goo.v1 = Goo.denormalize(dOutVec[0], -Goo.MAX_SPEED_VEC, Goo.MAX_SPEED_VEC)*goo.speedFactor;
					goo.v2 = Goo.denormalize(dOutVec[1], -Goo.MAX_SPEED_VEC, Goo.MAX_SPEED_VEC)*goo.speedFactor;
    		...
    		if(goos.size()<=WINNER_CNT || timecnt>=Goo.MAX_LIFE*10){
    			...
    			for(int i=0; i<goocnt; i++){
    				...
    				if(winners.size()>0 && i<maxwinnerchild){ //每个繁殖3个个体
    					...
    					if(Math.random()<0.5){
    						newgoo = new MemGoo(IDPool.getID() , winners.get(idx), getSize().width, getSize().height);//完整保留基因
    					}else{    						
    						newgoo = MemGoo.mate(IDPool.getID(), winners.get(idx), winners.get(idx)); //自我复制
    						newgoo = new MemGoo(newgoo.ID , newgoo, getSize().width, getSize().height);//重新定义位置
    					}
    				}else{
    					int j = i % WEIGHTS.length;
    					if(DUEL){
    						newgoo = new MemGoo(-1*(j+1), getSize().width, getSize().height, initGenes[j], true); //使用已保存的个体
    						Random rand = new Random(newgoo.ID);
    						rand.nextInt();
    						Color color = new Color(rand.nextInt());
    						newgoo.ball.c = color;
    					}else{
...
//						goo.target.life-=100;
					...
					if(goo.eatValue > 0.5f 
						...
						if(goo.attrSTR >= goo.target.attrSTR ){
							...
							goo.attrSTR += Math.max((goo.target.attrSTR*0.5f), Goo.DELTA_ATTR);  //吃掉对方一半的体力
							if(goo.attrSTR > Goo.MAX_ATTR)
								goo.attrSTR = Goo.MAX_ATTR;
							goo.ball.size = (int) goo.attrSTR;
							goo.speedFactor = 1.0f/goo.attrSTR; //变大后变慢 （AGL下降）
							...
							if(goo.score > bestScore)
								bestScore=goo.score;
							eatCnt++;
//							if(goo.target instanceof Food)
//								System.out.println("goo "+goo.ID + " eat food ");
//							else
//								System.out.println("goo "+goo.ID + "Killed "+goo.target.ID);
						}
...
//		 File file=new File(path);
	...
	public static void main(String[] args){
		JApplet myApplet=new SpecialGooWar();
